...
#0 0 0.37367    −0.08896
#0 2 −0.01411   −0.00470
#get similar values, but not make sense in physics: −0.01411 / 0.37367 ~ -0.03776, −0.00470 / −0.08896 ~ 0.0528; -0.01048 / 0.06289 ~ -0.1666
...
def EsGB_corrections_Carson_Yagi(name):
    ...
    return corr
# data given in 2102.05939 (TABLE 1), 2002.08559 (TABLE 2)

# EFT
def EFT_corrections(name, dim, EFT):

    """

    This function returns the coefficients of the Parspec expansion for EFT corrections to the (l,m,n)=(2,2,0) mode.

    Parameters
    ----------

    name : str
        Name of the mode deviation ('domega_220' or 'dtau_220').
    dim : int
        Number of terms to be used in the Parspec expansion.
    EFT : str
        EFT sector: 
    ...
    corr : array
        Coefficients of the Parspec expansion (polar, no axial).

    """

    # === Parspec coefficients for each EFT sector ===
    # (You must fill in with the correct numerical values)

    all_corr = {

        'cubic_even': {
            'domega_220': [-0.144, 0.572, 1.389],
            'dtau_220'  : [0.162, 0.411, -0.858],
        },

        'cubic_odd': {
            'domega_220': [0.197, -0.422, -1.431],
            'dtau_220'  : [-0.151, -0.512, 0.561],
        },

        'quartic_1': {
            'domega_220': [-0.166, -0.715, -2.145],
            'dtau_220'  : [-0.244, -0.451, 0.176],
        },

        'quartic_2': {
            'domega_220': [0.000, -0.100, 1.104],
            'dtau_220'  : [0.000, 0.415, 0.019],
        },

        'quartic_3': {
            'domega_220': [0.074, 0.252, 1.625],
            'dtau_220'  : [0.136, 0.527, 0.280],
        }
    }

    # --- Safety checks ---
    if EFT not in all_corr.keys():
        raise ValueError("Unknown EFT model. Allowed: cubic_even, cubic_odd, quartic_even, quartic_odd.")

    if name not in all_corr[EFT].keys():
        raise ValueError("Currently EFT corrections only support the (l,m,n)=(2,2,0) mode deviation.")

    corr = []
    for i in range(dim+1):
        corr.append(all_corr[EFT][name][i])

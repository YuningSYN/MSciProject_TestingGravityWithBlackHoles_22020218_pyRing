...
#ell_units_factor is used to multiply ell to get a dimensionless quantity: (c^2 * 10^3) / (M_{sun} * G) --> unit: (m^{2} s^{-2}) / (kg m^{3} kg^{-1} s^{-2}) = m^{-1} --> ell[m] * [m^{-1}] is dimensionless since 10^3 in ell[km] is cancelled
...
class LIGOVirgoModel(cpnest.model.Model):
    ...
    def __init__(self, **kwargs):
        ...
        # Estimate of peak time in reference detector. --> merger time
...
#父类：LIGOVirgoModel → 数据 + 探测器 + sky prior + noise evidence
#子类负责：ringdown waveform + log likelihood
class IMR_Ringdown_Model(RingDownModel):
    ...
    def __init__(self, **kwargs):
        ...
        self.IMR_approx = kwargs['template'].strip('IMR-')
        ...
        for name in self.default_bounds.keys():
            try:
                ...
                single_bounds = read_parameter_bounds(self.Config, configparser, kwargs, name, self.default_bounds, self.IMR_approx)
                self.names.append('{}'.format(name))
                self.bounds.append(single_bounds)
...
class KerrModel(RingDownModel):
    ...
    def __init__(self, **kwargs):
        ...
        if(self.EFT):
            ...
            # no specific limit for ParSpec_Dmax_TGR and ParSpec_Dmax_charge
            
            self.EFT_corrections_dict = {}
            self.EFT_corrections_dict['domega_220'] = utils.EFT_corrections('domega_220', self.ParSpec_Dmax_TGR, self.EFT)
            self.EFT_corrections_dict['dtau_220']   = utils.EFT_corrections('dtau_220'  , self.ParSpec_Dmax_TGR, self.EFT)
        ...
        # EFT added
        if(self.EsGB or (self.ParSpec and (self.ParSpec_Dmax_charge > 0)) or self.EFT):
            ...
            if(  'cubic'   in self.EFT): self.EFT_power = 4
            elif('quartic' in self.EFT): self.EFT_power = 6
            else                              : raise ValueError(f'Unknown EFT value ({self.EFT}).')
        ...
        # Read beyond-Kerr parameters (EFT added)
            ...
            if (self.domega_tgr_modes is not None):
                for mode in self.domega_tgr_modes:
                    ...
                    #use ParSpec
                        ...
                        if not(self.EsGB) and not(self.EFT):
        ...
        # EFT added
            ...
            utils.review_warning()
            EFT_default_bounds = [-740,740] #km units, consistent with the default maximum mass prior of 500Msun
            EFT_ell = 'ell'
            try:
                self.fixed_params[EFT_ell] = self.Config.getfloat("Priors",'fix-'+EFT_ell)
            except(configparser.NoOptionError):
                single_bounds = [0.0,0.0]

                try                                                                      : single_bounds[0] = self.Config.getfloat("Priors",EFT_ell+'-min')
                except(KeyError, configparser.NoOptionError, configparser.NoSectionError): single_bounds[0] = eft_default_bounds[0]
                try                                                                      : single_bounds[1] = self.Config.getfloat("Priors",EFT_ell+'-max')
                except(KeyError, configparser.NoOptionError, configparser.NoSectionError): single_bounds[1] = eft_default_bounds[1]

                print(('{} : [{},{}]'.format(EFT_ell.ljust(len('cos_altitude')), single_bounds[0], single_bounds[1])))
                if (kwargs['inject-EFT']):
                    if (not(single_bounds[0] <= kwargs['injection-parameters'][EFT_ell] <= single_bounds[1])): utils.print_out_of_bounds_warning(EFT_ell)
                self.names.append(EFT_ell)
                self.bounds.append(single_bounds)
            
            if not(self.domega_tgr_modes==self.dtau_tgr_modes): raise ValueError('The domega and dtau modes must be the same when considering EFT corrections.')
        ...
        # Read EFT parameters (similar to process of tail parameters)
        if (self.EFT):
            utils.print_subsection('EFT')

            EFT_amps, EFT_phis = self.EFT_amp_names()
            for name in np.concatenate((EFT_amps, EFT_phis), axis=0):
                name = str(name)
                try:
                    self.fixed_params[name] = self.Config.getfloat("Priors",'fix-'+name)
                except(configparser.NoOptionError):
                    try:
                        ...
                        # These are relative variations with respect to the GR modes, hence the different conventions compared to standard amplitudes and phases (which are respectively positive, and defined in [0, 2pi]).
                        if ('phi-EFT-' in name): single_bounds = [-np.pi, +np.pi]
                        elif ('A-EFT-' in name): single_bounds = [-1.0, +1.0]
                        else                   : raise ValueError("* Kerr EFT section: you tried to pass an unknown parameter.")
                    self.names.append(name)
                    self.bounds.append(single_bounds)
                    print(('{} : [{}, {}]'.format(name.ljust(len('cos_altitude')), single_bounds[0], single_bounds[1])))
...
#EFTofGR (we only need to compare the results (for different events) with the results in published papers using other events, so don't need to sampling 'ell')
#        if(not(self.EFT_theory=='')):
#            logdist_tmp = get_param_override(self.fixed_params, x, 'logdistance')
#            ell_tmp = get_param_override(self.fixed_params, x, 'ell')
#            af_tmp = get_param_override(self.fixed_params, x, 'af')
#            Mf_tmp = get_param_override(self.fixed_params, x, 'Mf')
#            z_tmp = ((np.exp(logdist_tmp)*lal.PC_SI*1e6)/lal.C_SI)*lal.H0_SI
#            gamma_tmp = np.sign(ell_tmp) * ((ell_units_factor*np.abs(ell_tmp)*(1+z_tmp)) / Mf_tmp) ** (self.EFT_power)
#            tau_factor = (lal.C_SI * lal.C_SI * lal.C_SI / (lal.G_SI * lal.MSUN_SI))
#            Mft = Mf_tmp / tau_factor
#            if(np.abs(gamma_tmp) > 1): return -np.inf
#            if(self.EFT_exp_growth_cutoff  > 0): self.EFT_exp_growth_cutoff  = -np.inf
#            if(self.EFT_exp_damping_cutoff < 0): self.EFT_exp_damping_cutoff = +np.inf
#            for mode in self.dtau_tgr_modes:
#                (l, m, n) = mode
#               tau_plus  = wf.QNM_EFT_fit(l, m, n, self.EFT_theory + "_plus" ).tau_EFT(Mf_tmp, af_tmp, gamma_tmp)
#                tau_minus = wf.QNM_EFT_fit(l, m, n, self.EFT_theory + "_minus").tau_EFT(Mf_tmp, af_tmp, gamma_tmp)
#                if(np.minimum(Mft/tau_plus, Mft/tau_minus)/(n+1/2) < self.EFT_exp_growth_cutoff or
#                  np.maximum(Mft/tau_plus, Mft/tau_minus)/(n+1/2) > self.EFT_exp_damping_cutoff): return -np.inf
    ...
    # EFT added
    def EFT_amp_names(self):

        # Initialise structures
        EFT_amps, EFT_phis = [], []
        #if(self.EFT_no_amp_split): return EFT_amps, EFT_phis
        for (l,m,n) in self.EFT_modes:
        
            EFT_string = '{}{}{}'.format(l,m,n)

            if(self.amp_non_prec_sym):
                EFT_amps.append(  'A-EFT-'+EFT_string)
                EFT_phis.append('phi-EFT-'+EFT_string)

            else:
                EFT_amps.append(  'A-EFT-'+EFT_string+'_1')
                EFT_amps.append(  'A-EFT-'+EFT_string+'_2')
                EFT_phis.append('phi-EFT-'+EFT_string+'_1')
                EFT_phis.append('phi-EFT-'+EFT_string+'_2')

        return EFT_amps, EFT_phis 
...
#EFTofGR (we only need to compare the results (for different events) with the results in published papers using other events)

#    def get_EFT_parameters(self,x,EFT_string):
#
#        EFT_dict = {}

        # The s=2 polarisation is assumed here

#        if (self.amp_non_prec_sym):
#
#            amp = get_param_override(self.fixed_params, x, 'A2' + EFT_string)
#            phi = get_param_override(self.fixed_params, x, 'phi2' + EFT_string)

            #if (self.EFT_no_amp_split):
            #    EFT_dict['A_p'] = 1 / 2 * amp
            #    EFT_dict['phi_p'] = phi
            #    EFT_dict['A_m'] = 1 / 2 * amp
            #    EFT_dict['phi_m'] = phi

            #else:
#            d_amp = get_param_override(self.fixed_params, x, 'A-EFT-' + EFT_string)
#            d_phi = get_param_override(self.fixed_params, x, 'phi-EFT-' + EFT_string)
#
#            EFT_dict['A_p'] = 1 / 2 * amp * (1 + d_amp)
#            EFT_dict['phi_p'] = phi + d_phi / 2
#            EFT_dict['A_m'] = 1 / 2 * amp * (1 - d_amp)
#            EFT_dict['phi_m'] = phi - d_phi / 2
#
#        else:
#            amp_1 = get_param_override(self.fixed_params, x, 'A2' + EFT_string + '_1')
#            amp_2 = get_param_override(self.fixed_params, x, 'A2' + EFT_string + '_2')
#            phi_1 = get_param_override(self.fixed_params, x, 'phi2' + EFT_string + '_1')
#            phi_2 = get_param_override(self.fixed_params, x, 'phi2' + EFT_string + '_2')

            #if (self.EFT_no_amp_split):
            #    EFT_dict['A_p_1']   = 1/2 * amp_1
            #    EFT_dict['A_p_2']   = 1/2 * amp_2
            #    EFT_dict['A_m_1']   = 1/2 * amp_1
            #    EFT_dict['A_m_2']   = 1/2 * amp_2

            #    EFT_dict['phi_p_1'] = phi_1
            #    EFT_dict['phi_p_2'] = phi_2
            #    EFT_dict['phi_m_1'] = phi_1
            #    EFT_dict['phi_m_2'] = phi_2

            #else:
#            d_amp_1 = get_param_override(self.fixed_params, x, 'A-EFT-' + EFT_string + '_1')
#            d_amp_2 = get_param_override(self.fixed_params, x, 'A-EFT-' + EFT_string + '_2')
#            d_phi_1 = get_param_override(self.fixed_params, x, 'phi-EFT-' + EFT_string + '_1')
#            d_phi_2 = get_param_override(self.fixed_params, x, 'phi-EFT-' + EFT_string + '_2')
#
#            EFT_dict['A_p_1'] = 1/2 * amp_1 * (1 + d_amp_1)
#            EFT_dict['A_p_2'] = 1/2 * amp_2 * (1 + d_amp_2)
#            EFT_dict['A_m_1'] = 1/2 * amp_1 * (1 - d_amp_1)
#            EFT_dict['A_m_2'] = 1/2 * amp_2 * (1 - d_amp_2)
#
#            EFT_dict['phi_p_1'] = phi_1 + d_phi_1 / 2
#            EFT_dict['phi_p_2'] = phi_2 + d_phi_2 / 2
#            EFT_dict['phi_m_1'] = phi_1 - d_phi_1 / 2
#            EFT_dict['phi_m_2'] = phi_2 - d_phi_2 / 2
#
#        return EFT_dict
    ...
    def get_waveform(self,x):
        ...
        amps, quad_amps, tail_parameters, EFT_parameters, TGR_parameters= {}, {}, {}, {}
        ...
        # Build the EFT parameters.
#        if(self.EFT_modes is not None):
#            for (l,m,n) in self.EFT_modes: 
#                EFT_parameters[(l,m,n)] = self.get_EFT_parameters(x,'{}{}{}'.format(l,m,n))
        ...
        # EFT added
#        elif(not(self.EFT=='')):
#            logdistance = get_param_override(self.fixed_params, x, 'logdistance')
#            ell = get_param_override(self.fixed_params, x, 'ell')
#            Mf = get_param_override(self.fixed_params, x, 'Mf')
#            z = ((np.exp(logdistance)*lal.PC_SI*1e6)/lal.C_SI)*lal.H0_SI
            #Hubble's Law - low redshift approximation: z ~ d_L[Mpc] * H_0 / c = (e^{log(d_L)} * Mpc)[m] * H_0[s^{-1}] / c[m s^{-1}] --> z is dimensionless
            # ell is measured in km, can be both positive and negative
#            TGR_parameters['gamma'] = np.sign(ell)*((ell_units_factor*np.abs(ell)*(1+z))/Mf)**(self.EFT_power)
            #arXiv-2205.05132: \gamma = \left(\frac{\ell_{\rm th}}{M_{\rm f}^{\rm s}}\right)^p = \left[\frac{\ell_{\rm th} c^2 (1 + z)}{G M_{\rm f}}\right]^p
#            for name in self.TGR_params: TGR_parameters[name] = np.array(self.EFT_corrections_dict[name])
        else:
            if not(self.ParSpec):
                ...
                if (self.ParSpec_Dmax_charge > 0):
                    ...
                    if (self.EFT):  #ParSpec_Dmax_charge = EFT_power
                        logdistance = get_param_override(self.fixed_params, x, 'logdistance')
                        ell = get_param_override(self.fixed_params, x, 'ell')
                        Mf = get_param_override(self.fixed_params, x, 'Mf')
                        z = ((np.exp(logdistance)*lal.PC_SI*1e6)/lal.C_SI)*lal.H0_SI
                        TGR_parameters['gamma'] = np.sign(ell)*((ell_units_factor*np.abs(ell)*(1+z))/Mf)**(self.ParSpec_Dmax_charge)
                    else:
                        z = ((np.exp(x['logdistance'])*lal.PC_SI*1e6)/lal.C_SI)*lal.H0_SI
                ...
                if (self.EsGB):
                    ...
                    for name in self.TGR_params:
                        TGR_parameters[name] = np.array(self.EFT_corrections_dict[self.EFT][name])
                else:
                    for name in self.TGR_params:
                        TGR_parameters[name] = []
...
def main():
    ...
    try:
        if not(input_par['run-type']=='post-processing'):
    ...
    elif ('IMR' in input_par['template']):                 signal_model = IMR_Ringdown_Model( **input_par)
    ...
    if(input_par['sampler']=='cpnest'):
        ...
        sampler_params={'resume'                      : 1, 
                        'logging_interval'            : 10800,
                        'periodic_checkpoint_interval': 10800,
                        'seed'                        : 1234,
                        'n_pool'                      : 1,
                        'nlive'                       : 1024,
    ...
    for key in sampler_params:
        try:
            ...
            elif isinstance(sampler_params[key], float):
                sampler_params[key] = Config.getfloat("Sampler settings", key)
            # else: leave as default (no string/bool parsing here by design)
        except (KeyError, configparser.NoOptionError, configparser.NoSectionError, ValueError):
            # Not present or malformed -> keep the default
            pass
        print("{name} : {value}".format(name=key.ljust(len('periodic_checkpoint_interval')),value=sampler_params[key]))    
    
    if(input_par['sampler']=='cpnest' or input_par['sampler']=='raynest'):
        ...
        if(sampler_params['checkpoint_interval']==0): sampler_params['checkpointing'] = False
        else: sampler_params['checkpointing'] = True
    ...
    if input_par['sampler'] == 'cpnest':
        ...
        sampler_outfiles = [os.path.join(input_par['output'],'Nested_sampler','result.hdf5')]
    else:
    ...
    if not(input_par['run-type']=='post-processing') and not sampler_complete:
        ...
        if input_par['sampler']=='cpnest':
            ...
            print('* The                                                                                                                             output appears in the `{}/Nested_sampler/raynest.log` file.\n'.format(input_par['output']))
            ...
            try   : import nessai
            except: raise Exception('The nessai sampler requires the nessai package. Please see the nessai documentation for instilation instructions: https://nessai.readthedocs.io/en/latest/installation.html')
            print('* Using nessai version: `{}`.\n'.format(nessai.__version__))
            from nessai.flowsampler import FlowSampler
            from nessai.utils.logging import configure_logger
            from pyRing.nessai_utils import NessaiWrapper
            
            nessai_signal_model = NessaiWrapper(signal_model)
            logger = configure_logger(output = input_par['output']+'/Nested_sampler',include_logger_name=True)
            
            flow_config = {'n_blocks' : sampler_params['n_blocks'],
                           'n_neurons': sampler_params['n_neurons'],
                           'n_layers' : sampler_params['n_layers'],
                             }
            work = FlowSampler(nessai_signal_model,
                               output              = input_par['output']+'/Nested_sampler',
                               resume              = sampler_params['resume'],
                               logging_interval    = sampler_params['logging_interval'],
                               checkpoint_interval = sampler_params['periodic_checkpoint_interval'],
                               checkpointing       = sampler_params['checkpointing'],
                               seed                = sampler_params['seed'],
                               n_pool              = sampler_params['n_pool'],
                               nlive               = sampler_params['nlive'],
                               stopping            = sampler_params['stopping'],
                               plot                = sampler_params['plot'],
                               proposal_plots      = sampler_params['proposal_plots'],
                               reset_flow          = sampler_params['reset_flow'],
                               volume_fraction     = sampler_params['volume_fraction'],
                               flow_config         = flow_config)
            work.run() 

            result_file = input_par['output']+'/Nested_sampler/result.hdf5'
            with h5py.File(result_file, "r") as f:
                x = f["posterior_samples"][()]   
                logZ_signal = f["log_evidence"][()] 
    ...
    #======================================#
    # Move plots to appropriate directory. #
    #======================================#
    ...
    if(input_par['run-type']=='sampling-only'):
        execution_time = (time.time() - start_execution_time)/60.0
        print('* Total execution time (min): {:.2f}'.format(execution_time))
        print('* Sampling and minimal plots done, `sampling-only` option was selected. Exiting.')
        exit()
    ...
    if(input_par['pesummary']):
        ...
        try   : import pesummary
        except: raise Exception('PESummary package not found. If you wish to use this functionality, please install it following the instructions at https://pesummary.readthedocs.io/en/latest/installation.html')
            ...
            if not(input_par['screen-output'] or (input_par['run-type']=='post-processing')):
                try:
                    ...
                    if(input_par['sampler']=='cpnest'):
                        ...
                        if os.path.exists('{outdir}/Nested_sampler/nessai.log'.format(outdir=input_par['output'])): H_file = '{outdir}/Nested_sampler/nessai.log'.format(outdir=input_par['output'])
                        else                                                                                      : H_file = '{outdir}/stderr_pyRing.txt'.format(         outdir=input_par['output'])
